...
public class TSDES {
	...
	public static void main(String[] args) {
		...
		System.out.println("==================================================== TSDES Program ===================================================");
		...
		for(int i = 0; i < 4; i++) {
			byte[] ciphertextResult = Encrypt(rawkey1[i], rawkey2[i], plaintext[i]);
			ciphertextResult = Encrypt(rawkey2[i], rawkey1[i], ciphertextResult);
			ciphertext[i] = Encrypt(rawkey1[i], rawkey2[i], ciphertextResult);
			System.out.println(Arrays.toString(rawkey1[i]) + "   " + Arrays.toString(rawkey2[i])  + "   " + Arrays.toString(plaintext[i]) + "   " + Arrays.toString(ciphertext[i]));
		}
		...
		for(int i = 0; i < 4; i++) {
			byte[] plaintextResult = Decrypt(rawkey1[i], rawkey2[i], ciphertext[i]);
			plaintextResult = Decrypt(rawkey2[i], rawkey1[i], plaintextResult);
			plaintext[i] = Decrypt(rawkey1[i], rawkey2[i], plaintextResult);
			System.out.println(Arrays.toString(rawkey1[i]) + "   " + Arrays.toString(rawkey2[i])  + "   " + Arrays.toString(plaintext[i]) + "   " + Arrays.toString(ciphertext[i]));
		}
	...
	private static byte[] Decrypt(byte[] rawkey1, byte[] rawkey2, byte[] ciphertext) {
		// TODO Auto-generated method stub
		byte[] p10key = P10(rawkey1);
	...
	private static byte[] Encrypt(byte[] rawkey1, byte[] rawkey2, byte[] plaintext) {
		// TODO Auto-generated method stub
		byte[] p10key = P10(rawkey1);
		byte[] keyLS1 = keyLS1(p10key);
		byte[] subKey1 = P8(keyLS1);
